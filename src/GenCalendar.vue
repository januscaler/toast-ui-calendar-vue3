<template>
	<div :style="{ '--timezone-text-color': darkMode ? darkTextColor : lightTextColor }" class="tw:h-[inherit]" :class="{ dark: props.darkMode }">
		<div :style="{ backgroundColor: props.darkMode ? props.darkBgColor : props.lightBgColor }" class="tw:flex tw:flex-col tw:w-full tw:h-[inherit]">
			<div class="tw:flex tw:items-center tw:p-[10px] tw:gap-x-[10px]">
				<v-btn :style="{ color: darkMode ? darkTextColor : lightTextColor }" variant="outlined" flat @click="goToToday" rounded style="outlined">{{ $t('today') }}</v-btn>
				<v-btn @click="prev" elevation="0" color="transparent" size="30px" icon>
					<v-icon :style="{ color: darkMode ? darkTextColor : lightTextColor }" size="22px" icon="fas fa-chevron-left" />
				</v-btn>
				<v-btn @click="next" elevation="0" color="transparent" size="30px" icon>
					<v-icon :style="{ color: darkMode ? darkTextColor : lightTextColor }" size="22px" class="tw:dark:text-white tw:text-black" icon="fas fa-chevron-right" />
				</v-btn>
				<p class="tw:w-[120px]" :style="{ color: darkMode ? darkTextColor : lightTextColor }">
					{{ currentSelection }}
				</p>
				<v-select
					:bg-color="darkMode ? darkBgColor : lightBgColor"
				...
				<GenCalendarSettings
					:dark-mode="darkMode"
					v-model:primary-timezone="primaryTimezone"
					v-model:secondary-timezone="secondaryTimezone"
					:time-zones="allTimeZones"
					:save-form="saveForm"
					v-model="modalValue"
				>
					<template v-slot:activator="{ props: activatorProps }">
						<v-btn v-bind="activatorProps" @click="settings" elevation="0" color="transparent" size="30px" icon>
